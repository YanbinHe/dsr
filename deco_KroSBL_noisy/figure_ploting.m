...
resultSrraggre = zeros(7,length(M1));
...
%         end
        ...
        for m = 1:length(M1)
            ...
            resultSrraggre(algo,m) = resultSrraggre(algo,m) + 1*(resultM{m}{algo,1}{metric,2} == 1);
        end
...
resultSrraggre = resultSrraggre/trials;
...
figure
for algo_index = 1:num_alg
    plot(ratio,resultSrraggre(algo_index,:),line_type_set{algo_index},'Color',all_colors(algo_index, :),'Display',algo_name{algo_index});
    hold on
end
...
set(get(gca,'Title'),'FontWeight','bold')
ylabel('Success rate')
ylabel('Under-sampling ratio')
% axis square

h1 = plot(ratio,resultSrraggre(1,:),legend_type_set{1},'Color',all_colors(1, :),'Display',algo_name{1});
hold on
h2 = plot(ratio,resultSrraggre(2,:),legend_type_set{2},'Color',all_colors(2, :),'Display',algo_name{2});
hold on
h3 = plot(ratio,resultSrraggre(3,:),legend_type_set{3},'Color',all_colors(3, :),'Display',algo_name{3});
hold on
h4 = plot(ratio,resultSrraggre(4,:),legend_type_set{4},'Color',all_colors(4, :),'Display',algo_name{4});
hold on
% h5 = plot(ratio,reshape(resultMaggre(5,metric,:),[6,1]),legend_type_set{5},'Color',all_colors(5, :),'Display',algo_name{5});
% hold on
legend([h1 h2 h3 h4],{algo_name{1},algo_name{2},algo_name{3},algo_name{4}},'Location','northeast','Interpreter','LaTex')
saveas(gcf,'deco1','epsc')
